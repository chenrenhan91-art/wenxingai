...
def redact_url(url: str) -> str:
    ...
    for key, value in query:
        if key.lower() in {"access_token", "api_key", "token", "authorization"}:
...
def fetch_graphql(api_key: str, query: str, variables: dict[str, Any] | None = None) -> dict[str, Any]:
    payload = json.dumps(
    ...
    if not isinstance(response, dict):
        raise RuntimeError("invalid Buffer GraphQL response")
    errors = response.get("errors") or []
    if errors:
        messages = [
            str(item.get("message") or "unknown Buffer GraphQL error")
            for item in errors
            if isinstance(item, dict)
        ]
        raise RuntimeError("; ".join(messages) or "unknown Buffer GraphQL error")
    data = response.get("data")
    if not isinstance(data, dict):
        raise RuntimeError("invalid Buffer GraphQL data payload")
    return data
...
def get_buffer_organization_id(api_key: str) -> str:
    ...
    if len(organizations) > 1:
        raise RuntimeError("multiple Buffer organizations found; set BUFFER_ORGANIZATION_ID")
    organization_id = str((organizations[0] or {}).get("id") or "").strip()
    if not organization_id:
        raise RuntimeError("Buffer organization response is missing id")
    return organization_id


def get_buffer_profiles(api_key: str) -> list[dict[str, Any]]:
    organization_id = get_buffer_organization_id(api_key)
    data = fetch_graphql(
        api_key,
        """
...
def resolve_profile_id(
    ...
    matched = [
        ...
        and not bool(profile.get("isDisconnected"))
        and not bool(profile.get("isLocked"))
    ]
    if not matched:
...
def publish_jobs_to_buffer(jobs: list[dict[str, Any]], state: dict[str, Any]) -> tuple[list[dict[str, Any]], int]:
    ...
    for job in jobs:
        ...
        explicit_profile_id = os.getenv(PLATFORM_TO_ENV[job["platform"]], "").strip()
        if not explicit_profile_id and profiles is None:
