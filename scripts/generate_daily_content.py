...
CONTENT_SCHEMA: dict[str, Any] = {
    ...
    "properties": {
        ...
        "social_posts": {
            ...
            "required": ["threads", "x", "facebook", "instagram"],
        },
        "video_script": {
            "type": "object",
            "properties": {
        ...
        "distribution_plan": {
            ...
            "required": ["threads", "x", "facebook", "instagram", "notes"],
        },
    },
...
def validate_generated_content(content: dict[str, Any]) -> dict[str, Any]:
    site_article = content.get("site_article")
    social_posts = content.get("social_posts")
    video_script = content.get("video_script")
    distribution_plan = content.get("distribution_plan")
    if not isinstance(site_article, dict):
        raise RuntimeError("invalid Gemini response: missing site_article")
    if not isinstance(social_posts, dict):
        raise RuntimeError("invalid Gemini response: missing social_posts")
    if not isinstance(video_script, dict):
        raise RuntimeError("invalid Gemini response: missing video_script")
    if not isinstance(distribution_plan, dict):
        raise RuntimeError("invalid Gemini response: missing distribution_plan")

    validated = {
        "site_article": {
            "title": require_string(site_article, "title", "site_article"),
            "seo_title": require_string(site_article, "seo_title", "site_article"),
            "seo_description": require_string(site_article, "seo_description", "site_article"),
            "excerpt": require_string(site_article, "excerpt", "site_article"),
            "body_markdown": require_string(site_article, "body_markdown", "site_article"),
            "cta": require_string(site_article, "cta", "site_article"),
        },
        "social_posts": {
            "threads": require_string_list(social_posts, "threads", "social_posts"),
            "x": require_string_list(social_posts, "x", "social_posts"),
            "facebook": require_string_list(social_posts, "facebook", "social_posts"),
            "instagram": require_string_list(social_posts, "instagram", "social_posts"),
        },
        "video_script": {
            "title": require_string(video_script, "title", "video_script"),
            "hook": require_string(video_script, "hook", "video_script"),
            "script": require_string(video_script, "script", "video_script"),
            "cta": require_string(video_script, "cta", "video_script"),
        },
        "distribution_plan": {
            "threads": require_string(distribution_plan, "threads", "distribution_plan"),
            "x": require_string(distribution_plan, "x", "distribution_plan"),
            "facebook": require_string(distribution_plan, "facebook", "distribution_plan"),
            "instagram": require_string(distribution_plan, "instagram", "distribution_plan"),
            "notes": require_string(distribution_plan, "notes", "distribution_plan"),
        },
    }
    return validated
...
def render_markdown(bundle: dict[str, Any]) -> str:
    article = bundle["site_article"]
    social = bundle["social_posts"]
    video = bundle["video_script"]
    plan = bundle["distribution_plan"]
    ...
    lines.extend(["", "## Facebook 文案"])
    lines.extend(f"{index}. {post}" for index, post in enumerate(social["facebook"], start=1))
    lines.extend(["", "## Instagram 文案"])
    lines.extend(f"{index}. {post}" for index, post in enumerate(social["instagram"], start=1))
    lines.extend(
        [
            "",
            "## 短视频脚本",
